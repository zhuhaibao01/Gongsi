#coding=utf-8
import sys,unittest,requests
reload(sys)
sys.setdefaultencoding('utf-8')
sys.path.append(r'C:\Users\test\Desktop\jenkins\Base')
from CreatePhoneNum import *
class get_operators_inviteEnt(unittest.TestCase):


    def setUp(self):
        print u"#################自动执行测试用例开始#############"
        self.url='http://10.10.100.206/ucenter/member/partner/invite/ent'
        self.PartnerNotInvite= AutoMysqlPhoneIsOrNo('17600365340')
        self.PartnerInvite = AutoMysqlPhoneIsOrNo('13611346409')

    def tearDown(self):
        print u"#################自动执行测试结束##############"

    def test_01_allTrue(self):
        '''新媒人是被邀请用户的企业信息,userId 与invitedCode一致'''
        usr = get_IdOrToken(self.PartnerInvite)
        usrId = usr[0]
        usrheader = usr[2]
        content={'userId':usrId,'inviteCode':123457}
        r=requests.get(url=self.url,params=content,headers=usrheader)
        print r.content
        print r.json()
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()['code'], 200)
        self.assertEqual(r.json()['success'], True)
        self.assertEqual(r.json()['message'], u"请求成功")
        self.assertIsNotNone(r.json()['entity'])

    def test_02_allTrue(self):
        '''新媒人是被邀请用户的企业信息,invitedCode不存在'''
        usr = get_IdOrToken(self.PartnerInvite)
        usrId = usr[0]
        usrheader = usr[2]
        content = {'userId': usrId, 'inviteCode': 999999991}
        r = requests.get(url=self.url, params=content, headers=usrheader)
        print r.content
        print r.json()
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()['code'], 7007)
        self.assertEqual(r.json()['success'], False)
        self.assertEqual(r.json()['message'], u"邀请码不存在，请填写正确的邀请码！")
        self.assertIsNone(r.json()['entity'])

    def test_03_allTrue(self):
        '''新媒人是被邀请用户的企业信息,invitedCode为空'''
        usr = get_IdOrToken(self.PartnerInvite)
        usrId = usr[0]
        usrheader = usr[2]
        content = {'userId': usrId}
        r = requests.get(url=self.url, params=content, headers=usrheader)
        print r.content
        print r.json()
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()['code'], 7007)
        self.assertEqual(r.json()['success'], False)
        self.assertEqual(r.json()['message'], u"邀请码不存在，请填写正确的邀请码！")
        self.assertIsNone(r.json()['entity'])

    def test_04_allTrue(self):
        '''新媒人是被邀请用户的企业信息,invitedCode为特殊字符'''
        usr = get_IdOrToken(self.PartnerInvite)
        usrId = usr[0]
        usrheader = usr[2]
        content = {'userId': usrId, 'inviteCode':'~!@#$%^^*()'}
        r = requests.get(url=self.url, params=content, headers=usrheader)
        print r.content
        print r.json()
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()['code'], 7007)
        self.assertEqual(r.json()['success'], False)
        self.assertEqual(r.json()['message'], u"邀请码不存在，请填写正确的邀请码！")
        self.assertIsNone(r.json()['entity'])

    def test_05_allTrue(self):
        '''新媒人是被邀请用户的企业信息,invitedCode为汉字'''
        usr = get_IdOrToken(self.PartnerInvite)
        usrId = usr[0]
        usrheader = usr[2]
        content = {'userId': usrId, 'inviteCode':u'我是一个汉字'}
        r = requests.get(url=self.url, params=content, headers=usrheader)
        print r.content
        print r.json()
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()['code'], 7007)
        self.assertEqual(r.json()['success'], False)
        self.assertEqual(r.json()['message'], u"邀请码不存在，请填写正确的邀请码！")
        self.assertIsNone(r.json()['entity'])

    def test_06_allTrue(self):
        '''新媒人是被邀请用户的企业信息,invitedCode为空格'''
        usr = get_IdOrToken(self.PartnerInvite)
        usrId = usr[0]
        usrheader = usr[2]
        content = {'userId': usrId, 'inviteCode': u'我是一个    空格'}
        r = requests.get(url=self.url, params=content, headers=usrheader)
        print r.content
        print r.json()
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()['code'], 7007)
        self.assertEqual(r.json()['success'], False)
        self.assertEqual(r.json()['message'], u"邀请码不存在，请填写正确的邀请码！")
        self.assertIsNone(r.json()['entity'])

    def test_07_allTrue(self):
        '''新媒人是被邀请用户的企业信息,userId为空'''
        usr = get_IdOrToken(self.PartnerInvite)
        usrId = usr[0]
        usrheader = usr[2]
        content = {'inviteCode':123457}
        r = requests.get(url=self.url, params=content, headers=usrheader)
        print r.content
        print r.json()
        self.assertEqual(r.status_code,200)
        self.assertEqual(r.json()['code'], 2000)
        self.assertEqual(r.json()['success'], False)
        self.assertEqual(r.json()['message'], u"请求参数不正确")
        self.assertIsNone(r.json()['entity'])

if __name__ == '__main__':
    unittest.main()


