...
class member_modify_sendmessage(unittest.TestCase):


    def setUp(self):
        print u"#################自动执行测试用例开始#############"
        self.noregistum =int(C(sql(allSql()['sql1'])))
        self.url = 'http://10.10.100.206/ucenter/member/pwd/sendMessage'

    def tearDown(self):
        print u"#################自动执行测试结束##############"

    @unittest.skipUnless(False,u'数据库里的密码可能被改的不对')
    def test_01_mdtrue(self):
        ''' 发送正确的已注册的11位手机号'''
    ...
    def test_11_1minute(self):
        ''' 一分钟内发送验证码'''
        self.url = 'http://10.10.100.207:8085/ucenter/member/pwd/sendMessage'
        mylphone =sql(allSql()['sql1'])[random.randint(1,len(allSql()['sql1']))]
        mobile = {'mobile': mylphone}
        print mobile['mobile']
